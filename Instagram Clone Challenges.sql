...
LIMIT 1;
/*version 3 */
select  max(dd.count), dd.photo_id, a.username from (select photo_id, count(photo_id) as count from likes group by photo_id) as dd  
join photos c on c.id=dd.photo_id
join users a on a.id=c.user_id group by photo_id order by count desc limit 1;
...
SELECT SUM(user_posts.total_posts_per_user), user_posts.username
FROM (SELECT users.username,COUNT(photos.image_url) AS total_posts_per_user
		FROM users
		JOIN photos ON users.id = photos.user_id
		GROUP BY users.id) AS user_posts group by user_posts.username ORDER BY 1 DESC;
...
where comment_text IS NULL order BY username;
...
FROM
	...
	(
		SELECT COUNT(*) AS total_B FROM
			(SELECT users.id,username, COUNT(likes.user_id) As total_likes_by_user
				FROM users
				JOIN likes ON users.id = likes.user_id
				GROUP BY users.id
...
select  username, count(username) as ab from ( SELECT username,  comment_text
FROM users
LEFT JOIN comments ON users.id = comments.user_id

HAVING comment_text IS NOT NULL) as b group by username;               

/*Alternate1*/
SELECT username,  count(username) as aa
FROM users
...
(SELECT username,comment_text
	...
	HAVING comment_text IS NOT NULL) AS total_number_users_with_comments group by username;
...
FROM
	...
	JOIN
	(
		SELECT COUNT(distinct(username)) AS total_B FROM
			(SELECT username,comment_text
				FROM users
				LEFT JOIN comments ON users.id = comments.user_id
